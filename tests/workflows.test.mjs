...
function normalizedRun(step) {
  assert.equal(typeof step.run, 'string', `step ${step.name} must have run`);
...
test('reactions deploy is gated to a successful main Pages release or a main manual dispatch', async () => {
  const workflow = await readWorkflow('reactions-deploy.yml');
  const deploy = workflow.jobs.deploy;

  assert.equal(workflow.name, 'Deploy Reactions Worker');
...
test('reactions deploy checks out and verifies the exact released SHA', async () => {
  const deploy = (await readWorkflow('reactions-deploy.yml')).jobs.deploy;
  ...
  assert.deepEqual(stepNamed(deploy, 'Checkout'), {
    name: 'Checkout',
  ...
  assert.equal(
    normalizedRun(stepNamed(deploy, 'Verify release SHA')),
    'test "$(git rev-parse HEAD)" = "$RELEASE_SHA"',
  );
});

test('reactions deploy verifies locally before ordered additive remote operations and smoke', async () => {
  const deploy = (await readWorkflow('reactions-deploy.yml')).jobs.deploy;
  const orderedSteps = [
    'Checkout',
    'Verify release SHA',
  ...
  assert.deepEqual(
    orderedSteps.map((name) => stepIndex(deploy, name)),
    orderedSteps.map((_, index) => index),
  );
  assert.deepEqual(stepNamed(deploy, 'Setup Node'), {
    name: 'Setup Node',
  ...
  assert.equal(
    normalizedRun(stepNamed(deploy, 'Validate production API URL')),
    'node scripts/validate-reactions-env.mjs --required',
  );
  const validateIndex = stepIndex(deploy, 'Validate production API URL');
  const verifyReleaseIndex = stepIndex(
    deploy,
    'Verify current Pages release',
  );
  const migrationsIndex = stepIndex(deploy, 'List pending D1 migrations');
  assert.equal(verifyReleaseIndex, validateIndex + 1);
  assert.equal(migrationsIndex, verifyReleaseIndex + 1);

  const verifyRelease = normalizedRun(
    stepNamed(deploy, 'Verify current Pages release'),
  );
  const verifyReleaseCommand = verifyRelease.replace(/\\\s+/gu, '');
  assert.match(verifyRelease, /for attempt in \$\(seq 1 6\)/u);
  assert.match(
    verifyReleaseCommand,
    /gh run list --workflow deploy\.yml --branch main --status success --limit 1 --json databaseId,headSha/u,
  );
  assert.match(
    verifyRelease,
    /"\$latest_sha" == "\$RELEASE_SHA"/u,
  );
  assert.match(
    verifyRelease,
    /"\$latest_id" -ge "\$SOURCE_PAGES_RUN_ID"/u,
  );
  assert.match(verifyRelease, /sleep 5/u);
  assert.match(
    verifyRelease,
    /Current Pages release could not be verified\./u,
  );
  assert.equal(
    normalizedRun(stepNamed(deploy, 'List pending D1 migrations')),
...
test('reactions deploy never embeds secrets or destructive database commands', async () => {
  const source = await readFile(
    path.join(repoRoot, '.github', 'workflows', 'reactions-deploy.yml'),
    'utf8',
  );
  const deploy = (await readWorkflow('reactions-deploy.yml')).jobs.deploy;
  const runCommands = deploy.steps
    .filter((step) => typeof step.run === 'string')
    .map(normalizedRun)
    .join('\n');

  assert.doesNotMatch(source, /REACTION_HMAC_SECRET/);
  assert.doesNotMatch(runCommands, /\$\{\{\s*secrets\./);
  assert.doesNotMatch(
    runCommands,
    /\bwrangler\s+d1\s+execute\b|\b(reset|drop|truncate|delete)\b/i,
  );
  const cloudflareEnv = {
  ...
  for (const step of deploy.steps) {
    if (remoteSteps.has(step.name)) {
      assert.deepEqual(step.env, cloudflareEnv, step.name);
    } else if (step.name === 'Verify current Pages release') {
      assert.deepEqual(step.env, {
        GH_TOKEN: '${{ github.token }}',
        SOURCE_PAGES_RUN_ID: '${{ github.event.workflow_run.id }}',
      });
    } else {
      assert.equal(step.env, undefined, step.name);
    }
  }
});

test('Pages deploy validates the public Reaction API URL before the complete Astro verification pipeline', async () => {
  const workflow = await readWorkflow('deploy.yml');
  const build = workflow.jobs.build;
  const action = stepUsing(workflow.jobs.build, 'withastro/action@v3');

  assert.equal(workflow.name, 'Deploy to GitHub Pages');
  assert.deepEqual(workflow.on, {
    push: { branches: ['main'] },
    workflow_dispatch: null,
  });
  ...
  assert.deepEqual(workflow.jobs.deploy.concurrency, {
    group: 'production-release',
    'cancel-in-progress': false,
    queue: 'max',
  });
  ...
  assert.ok(
    stepIndex(build, 'Install dependencies')
      < stepIndex(build, 'Validate production reactions API'),
  );
  assert.ok(
    stepIndex(build, 'Validate production reactions API')
      < stepIndex(build, 'Build with Astro'),
  );
  assert.equal(
    normalizedRun(stepNamed(build, 'Validate production reactions API')),
    'node scripts/validate-reactions-env.mjs --required',
  );
...
test('content-check retains one required verify job running npm verify', async () => {
  const workflow = await readWorkflow('content-check.yml');

  assert.equal(workflow.name, 'Content Check');
    ...
    normalizedRun(
      workflow.jobs.verify.steps.find(
        (step) => (
          ...
          && normalizedRun(step) === 'npm run verify'
        ),
      ),
    ),
    'npm run verify',
...
test('reactions operations identify one exact manual run before watching it', async () => {
  const readme = await readFile(
    path.join(repoRoot, 'services', 'reactions', 'README.md'),
    'utf8',
  );
  const routine = readme.slice(
    readme.indexOf('## Routine deployment and read-only smoke'),
    readme.indexOf('## Migration and rollback rules'),
  );

  assert.doesNotMatch(routine, /--limit 1\b/u);
  for (const expected of [
    ...
    'More than one matching reactions workflow run was found.',
    'if [[ -z "$RUN_ID" ]]; then',
    'A matching reactions workflow run was not found in time.',
    'sleep 2',
    'gh run watch "$RUN_ID" --exit-status',
  ]) {
    assert.ok(routine.includes(expected), `missing operations guard: ${expected}`);
  }
  assert.ok(
    routine.indexOf('MAIN_SHA=')
      < routine.indexOf('DISPATCHED_AT='),
  );
  assert.ok(
    routine.indexOf('DISPATCHED_AT=')
      < routine.indexOf('gh workflow run'),
  );
  assert.ok(
    routine.indexOf('gh workflow run')
      < routine.indexOf('for attempt in'),
  );
  assert.ok(
    routine.indexOf('for attempt in')
      < routine.indexOf('gh run watch'),
  );
});
