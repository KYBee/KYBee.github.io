...
- **모바일에서 메시지 카드(`.message`) 레이아웃은 아바타 → 이름 → 내용이 세로로 쌓인다**
  (`flex-direction: column`, 모바일 미디어쿼리에서만). 데스크탑은 아바타·본문이 가로로 나란한 기존 레이아웃을
  그대로 유지한다. `.message` 클래스를 쓰는 모든 카드(경력·업무-프로젝트·사이드-프로젝트·기술·학력-자격)에
  공통 적용되므로 새 섹션에서도 별도 처리 없이 동일하게 동작한다
