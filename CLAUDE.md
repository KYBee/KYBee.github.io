...
- `# 경력`이 랜딩 뷰(첫 채널)이며 상단에 개인 소개 블록(channel-intro)이 온다
...
- `# 경력`의 회사 요약 메시지는 message-time에 기간만(`{group.period}`), 본문은 재직 중인 회사만 `about.work` 한 줄
  요약, 나머지는 `item.summary`를 사용한다
...
`# 경력` 상단 channel-intro 블록의 순서는 다음과 같다:
